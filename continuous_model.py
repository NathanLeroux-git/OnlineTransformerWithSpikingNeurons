...
class MLP1L(nn.Module):
    def __init__(self, args, dim_in, dim_out, hidden=500):
        super().__init__()
        self.linear = nn.Linear(args.window_size*dim_in, hidden)
        self.relu = nn.ReLU()
        self.linear2 = nn.Linear(hidden, dim_out)
        self.dropout = nn.Dropout(0.5)
        # self.bn = nn.BatchNorm1d(hidden, track_running_stats=False)
        self.window_size = args.window_size
        return

    def forward(self, x, batch_idx=1):    
        x = torch.flatten(x, 1)
        x = self.linear(x) 
        # x = self.bn(x) 
        x = self.dropout(x)
        x = self.relu(x)
        x = self.linear2(x)
        x = x.unsqueeze(2).expand(-1,-1,self.window_size)
        return x
...
class conv1d(nn.Module):
    def __init__(self, args, dim_in, dim_out):
        super().__init__()
        channel_in = [dim_in,16,32]
        channel_out = [16,32,64]
        dilated_conv_dilation = [2,4,8]
        causality_conv_stride = [1,2,4]
        FC_in = [channel_out[-1]*3, channel_out[-1]*4, 32]
        FC_out = [channel_out[-1]*4, 32, dim_out]
        self.net = nn.ModuleList([  # Conv blocks I, II and III
                                    nn.ModuleList([
                                        nn.Conv1d(channel_in[block], channel_out[block], 3, stride=1, padding="same", dilation=dilated_conv_dilation[block]),
                                        nn.BatchNorm1d(channel_out[block], track_running_stats=False),
                                        nn.ReLU(),
                                        nn.Conv1d(channel_out[block], channel_out[block], 3, stride=1, padding="same", dilation=dilated_conv_dilation[block]),
                                        nn.BatchNorm1d(channel_out[block], track_running_stats=False),
                                        nn.ReLU(),                                        
                                        nn.Conv1d(channel_out[block], channel_out[block], 5, stride=causality_conv_stride[block]),
                                    ...
                                    # Fully connected I and II
                                    [
                                    nn.ModuleList([
                                        nn.Linear(FC_in[fc], FC_out[fc]),
                                        nn.BatchNorm1d(FC_out[fc], track_running_stats=False),    
                                        nn.ReLU(), 
                                        nn.Dropout(p=0.5)   
                                        ])                                   
                                    for fc in range(2)                                        
    ...
    def forward(self, x, y):
        ...
        for module in self.net:
            for layer in module:
                x = layer(x)
        x = x.unsqueeze(-1).expand(-1,-1,time)
        return x
...
class EncoderDecoderTransformer(nn.Module):
    def __init__(self, args, dim_in, dim_out):
        ...
        if args.spiking_embedding:
            ...
            self.input_reshape = no_overlap_reshape(args, dim_in)
            self.input_decoder_reshape = no_overlap_reshape(args, dim_in)
            self.pos_embed = nn.Identity()    
            self.pos_drop = nn.Dropout(p=0)   
            self.upsampling_factor = self.patch_size
            PatchEmbed = getattr(continuous_model, 'ContinuousLIFPatchEmbed')
        else:
            self.input_reshape = overlap_reshape(args, dim_in)
            self.input_decoder_reshape = overlap_reshape(args, dim_in)           
            self.pos_embed = nn.Identity()   
            self.pos_drop = nn.Dropout(p=args.pos_drop_p) 
            self.upsampling_factor = self.input_reshape.stride
            PatchEmbed = getattr(continuous_model, 'ConvPatchEmbed')
            
        self.PatchEmbed = PatchEmbed(args, in_chans=dim_in)        
        # Encoder
        ...
        # Decoder
        self.SOS = nn.Parameter(torch.zeros(1, dim_out, 1), requires_grad=True)
        nn.init.uniform_(self.SOS, a=-1, b=+1)
        self.decoder_output = torch.zeros(1, dim_out, self.n_tokens+1, self.upsampling_factor).to(args.device)
        self.decoder_embedding = PatchEmbed(args, in_chans=dim_out)
        self.pos_drop_decoder = nn.Dropout(p=args.pos_drop_decoder_p) 
        self.blocks_decoder = nn.ModuleList(
            [
                transformer_block_decoder(args, continuous=False, mask_unseen_tokens=True)
                for _ in range(args.transfo_depth)
            ]
        )                
        self.classifier = MLP(args, args.embed_dim, args.classifier_hidden, dim_out) if not(args.spiking_classifier) else LIFNetwork(args, args.embed_dim, args.classifier_hidden, dim_out, readout_fn='U_readout')
    ...
    def forward(self, input, target=0):
        n_samples = input.shape[0]                     
        x = self.input_reshape(input)
        # Encoder
        x = self.PatchEmbed(x)
        x = self.pos_embed(x)
        x = self.pos_drop(x)
        encoder_out = self.encoder_step(x)          
        self.init_batch(input)
        for token in range(self.n_tokens):
            x = self.decoder_output.flatten(2)
            x = self.input_decoder_reshape(x)
            x = self.decoder_embedding(x)
            x = self.pos_embed(x)
            x = self.decoder_step(x, encoder_out)
            x = self.norm(x)
            x = self.NL(x)
            x = self.classifier(x)
            x = x.transpose(1,2)
            x = x.unsqueeze(-1).expand(-1,-1,-1,self.upsampling_factor)
            self.decoder_output[:,:,token+1] = x[:,:,token]                          
        output = self.decoder_output[:,:,1:]
    ...
    def decoder_step(self, x, encoder_out):    
    ...
    def init_batch(self, x):
        n_samples = x.shape[0]
        self.decoder_output = 0*self.decoder_output[0].detach() 
        self.decoder_output = self.decoder_output.expand(n_samples,-1,-1,-1).clone()  
        self.decoder_output[:,:,0] = self.SOS.expand(n_samples, -1, self.upsampling_factor)
        return
...
class ContinuousEncoderDecoderTransformer(nn.Module):
    def __init__(self, args, dim_in, dim_out):
        super().__init__()
        self.device = args.device
        self.in_channels = dim_in
        ...
        if args.spiking_embedding:
            assert args.window_size % args.n_tokens == 0
            self.patch_size = args.window_size // args.n_tokens
            self.input_reshape = no_overlap_reshape(dim_in, self.n_tokens, self.patch_size)
            self.pos_embed = nn.Identity()    
            self.pos_drop = nn.Dropout(p=0)   
            self.upsampling_factor = self.patch_size
            self.PatchEmbed = getattr(continuous_model, 'ContinuousLIFPatchEmbed')(args, in_chans=dim_in)      
        else:
            self.input_reshape = overlap_reshape(dim_in, self.window_size, self.n_tokens, args.conv_kernel_size, args.conv_stride, self.device)
            self.patch_size = 1            
            self.pos_embed = nn.Identity()   
            self.pos_drop = nn.Dropout(p=args.pos_drop_p) 
            self.upsampling_factor = self.input_reshape.stride
            self.PatchEmbed = getattr(continuous_model, 'ContinuousConvPatchEmbed')(args, in_chans=dim_in)        
        ...
        # Decoder
        self.decoder_init = nn.Parameter(torch.empty(1, dim_out), requires_grad=False)
        nn.init.uniform_(self.decoder_init, a=-1, b=+1)
        self.decoder_output = torch.zeros(1, dim_out)
        self.decoder_embedding = nn.Linear(dim_out, self.embed_dim)
        self.blocks_decoder = nn.ModuleList(
            [
                transformer_block_decoder(args)
                for _ in range(args.transfo_depth)
            ]
        )                
        self.classifier = MLP(args, args.embed_dim, args.classifier_hidden, dim_out) if not(args.spiking_classifier) else LIFNetwork(args, args.embed_dim, args.classifier_hidden, dim_out, readout_fn='U_readout')
    ...
    def forward(self,input,target=0):
        ...
        n_samples = input.shape[0]
        self.init_batch()   
        upsampled_output = torch.zeros(n_samples, self.dim_out, self.n_tokens, self.upsampling_factor).to(self.device)             
        input = self.input_reshape(input)
        for token in range(self.n_tokens):
            # Encoder
            x = input[:,:,token]
            x = self.PatchEmbed(x)
            ...
            x = x.unsqueeze(-1).expand(-1,-1,self.upsampling_factor)
            upsampled_output[:,:,token] = x                
        upsampled_output = upsampled_output.flatten(2)
    ...
    def init_epoch(self, x):
        ...
        # Transformer       
        for block in self.blocks_encoder:
            block.atten.K = torch.zeros(n_samples, block.atten.n_heads, block.atten.stored_vector_size, block.atten.head_dim).to(device)
            block.atten.V = torch.zeros(n_samples, block.atten.n_heads, block.atten.stored_vector_size, block.atten.head_dim).to(device)
            block.MLP_layers.init_state()   
        # Decoder   
        self.decoder_output = self.decoder_init.expand(n_samples, -1)         
        for block in self.blocks_decoder:
            block.atten.K = torch.zeros(n_samples, block.atten.n_heads, block.atten.stored_vector_size, block.atten.head_dim).to(device)
            block.atten.V = torch.zeros(n_samples, block.atten.n_heads, block.atten.stored_vector_size, block.atten.head_dim).to(device)
            block.cross_atten.K = torch.zeros(n_samples, block.atten.n_heads, block.atten.stored_vector_size, block.atten.head_dim).to(device)
            block.cross_atten.V = torch.zeros(n_samples, block.atten.n_heads, block.atten.stored_vector_size, block.atten.head_dim).to(device)
            block.MLP_layers.init_state()   
        # Classifier          
        self.classifier.init_state()
    ...
    def init_batch(self):
        # Embedding 
        self.PatchEmbed.Proj.detach_LIF_states()
        # Transformer
        for block in self.blocks_encoder:
            block.atten.K = block.atten.K.detach()
            block.atten.V = block.atten.V.detach()
            block.MLP_layers.detach_LIF_states()
        # Decoder  
        self.decoder_output = self.decoder_output.detach()
        for block in self.blocks_decoder:
            block.atten.K = block.atten.K.detach()
            block.atten.V = block.atten.V.detach()
            block.cross_atten.K = block.cross_atten.K.detach()
            block.cross_atten.V = block.cross_atten.V.detach()
            block.MLP_layers.detach_LIF_states()
        # Classifier 
        self.classifier.detach_LIF_states()  

class sine_cos_embed(nn.Module):
    def __init__(self, args) -> None:
        super().__init__()
        self.dim = args.embed_dim
        self.n_tokens = args.n_tokens
        self.dim_vector = torch.arange(0, self.dim).to(args.device)
        self.pos = 0
    def forward(self, x):
        x = x.clone() + int((self.pos % 2==0))*torch.cos(self.pos/10000**(2*self.dim_vector/self.dim)) + int((self.pos % 2==1))*torch.sin(self.pos/10000**(2*self.dim_vector/self.dim)) 
        self.pos += 1
        return x 
...
class transformer_block_decoder(nn.Module):
    def __init__(self, args, continuous=True, mask_unseen_tokens=False):
        super().__init__()
        self.norm1 = nn.LayerNorm(args.embed_dim) # if not(args.spiking_architecture) else nn.Identity()
        atten_fn = getattr(continuous_model, "Attention") if continuous else getattr(continuous_model, "ParallelAttention")
        self.atten = atten_fn(args)  
        self.norm2 = nn.LayerNorm(args.embed_dim) 
        self.norm2_encoder_out = nn.LayerNorm(args.embed_dim) 
        cross_atten_fn = getattr(continuous_model, "CrossAttention") if continuous else getattr(continuous_model, "ParallelCrossAttention")
        self.cross_atten = cross_atten_fn(args)  
    ...
    def forward(self, x, encoder_out):
        ...
        x = x + self.cross_atten(self.norm2(x), self.norm2_encoder_out(encoder_out))
        x = x + self.MLP_layers(self.norm3(x))
        return x
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, encoder_out):
        ...
        q, k, v = self.q_proj(x), self.k_proj(encoder_out), self.v_proj(encoder_out) # (n_samples, embed_dim) 
        q, k, v = q.reshape(n_samples, self.n_heads, self.head_dim), k.reshape(n_samples, self.n_heads, self.head_dim), v.reshape(n_samples, self.n_heads, self.head_dim)
        self.K[:,:,-1] = k
        self.V[:,:,-1] = v
        q = q.unsqueeze(2)
        dot_prod = torch.sum(q*self.K.clone(), dim=-1)
        dot_prod *= self.scale
        dot_prod[dot_prod==0] += float("-inf")
        dot_prod = self.softmax(dot_prod)
        dot_prod = dot_prod.unsqueeze(-1)
        weighted_average = torch.sum(dot_prod*self.V.clone(), dim=2)
        x = weighted_average.flatten(1) # (n_samples, n_heads*head_dim=embed_dim) flattens only above dimension 2 #
        x = self.proj(x) # (n_samples, head_embed_dim)
        x = self.proj_drop(x)
        return x